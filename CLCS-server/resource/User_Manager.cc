...
User_Manager* User_Manager::_pUser_manager = new User_Manager;
...
User* User_Manager::find_user(std::string & name) {
    // 如果输入的是名字， 则返回uuid
    if (_uuid_store.count(name) != 0) {
        name = _uuid_store[name];
    }

    // 查看uuid来获取用户信息
    if (_user_store.count(name) != 0) {
...
bool User_Manager::rename(std::string & user, std::string& name) {
