...
int Message_Sender::send_message(std::shared_ptr<Message_Stream> message) {
    int ret = send_message(message->get_header(), message->get_header_len());
    if (ret == -1)
        return ret;
    ret = send_message(message->get_content(), message->get_content_len());
    return ret;
}


int Message_Sender::send_message(std::shared_ptr<char[]> message, int len) {
    _sender.lock();
